...
if ($closeOnBrowserExit) {
  ...
  if ($browser -and $dshProc) {
    ...
    $browserProc = Start-Process -FilePath $browser -ArgumentList @('--app=' + $openUrl, '--user-data-dir=' + $browserProfile, '--no-first-run', '--no-default-browser-check') -PassThru
    ...
    Start-Process -FilePath $browser -ArgumentList @('--app=' + $openUrl, '--user-data-dir=' + $browserProfile, '--no-first-run', '--no-default-browser-check')
  } else {
