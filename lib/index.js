...
if (-not $alreadyRunning) {
  ...
  $pluginDeadline = (Get-Date).AddSeconds(5)
  do {
...
if ($closeOnBrowserExit) {
  ...
  if ($browser -and $dshProc) {
    ...
    $browserProc = Start-Process -FilePath $browser -ArgumentList @('--app=' + $openUrl, '--user-data-dir=' + $browserProfile, '--no-first-run', '--no-default-browser-check', '--disable-extensions', '--disable-background-networking', '--disable-sync') -PassThru
    ...
    Start-Process -FilePath $browser -ArgumentList @('--app=' + $openUrl, '--user-data-dir=' + $browserProfile, '--no-first-run', '--no-default-browser-check', '--disable-extensions', '--disable-background-networking', '--disable-sync')
  } else {
