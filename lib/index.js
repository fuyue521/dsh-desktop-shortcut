...
try {
  ...
  if (-not $alreadyRunning) {
    # Start dsh web in a hidden console via cmd /c (no visible window).
    ...
    # Wait until the client-plugin bundle route is ready too, but keep it
    # short and non-blocking: proceed after the timeout either way.
...
export function apply(ctx, config = {}) {
  ...
  const pluginCheckEnabled = config.pluginCheckEnabled !== false;
