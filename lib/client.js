...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const BG_FLAG = "dsh-desktop-shortcut-bg";

    // Restore the previous page background as early as possible so a dark
    // theme reload does not flash white before the app CSS loads.
    if (typeof document !== "undefined") {
      try {
        const bg = sessionStorage.getItem(BG_FLAG);
      ...
      const handleClick = () => {
        ...
        try {
          ...
          if (bg && bg !== "rgba(0, 0, 0, 0)" && bg !== "transparent") {
            sessionStorage.setItem(BG_FLAG, bg);
          }
        } catch {}
        setStatus("spinning");
        try {
